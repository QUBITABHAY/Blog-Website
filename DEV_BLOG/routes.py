    ...
    if form.validate_on_submit():
        try:
            existing_user = db.users.find_one(
                {"$or": [{"email": form.email.data}, {"username": form.username.data}]}
            )
...
def account():
    ...
    if form.validate_on_submit():
        if form.picture.data:
            ...
            db.posts.update_many(
                {"user_id": current_user.id}, {"$set": {"author_image": picture_file}}
            )
    ...
    recent_posts = list(
        db.posts.find({"user_id": current_user.id}).sort("date_posted", -1).limit(5)
    )
...
def new_post():
    ...
    if form.validate_on_submit():
        tags = (
            [tag.strip() for tag in form.tags.data.split(",")] if form.tags.data else []
        )

        safe_html = bleach.clean(form.content.data, tags=allowed_tags, strip=True)
...
def post(post_id):
    try:
        ...
        if post:
            return render_template(
                "post.html", title=post["title"], post=post, form=form
            )
        else:
        ...
        if post:
            ...
            if form.validate_on_submit():
                ...
                safe_html = bleach.clean(
                    form.content.data, tags=allowed_tags, strip=True
                )
                db.posts.update_one(
...
def user_posts(username):
    ...
    if user_data:
        posts = db.posts.find({"user_id": str(user_data["_id"])}).sort(
            "date_posted", -1
        )
